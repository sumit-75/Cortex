    ...
    <div className="bg-white border border-slate-200 rounded-2xl p-4 space-y-4 shadow-sm">
      <div className="flex items-center justify-between px-2">
        ...
        <button
          ...
          className="text-xs font-semibold text-indigo-600 hover:text-indigo-700 transition-colors cursor-pointer flex items-center gap-1"
        >
      ...
      {showAddForm && (
        <form onSubmit={handleCreateFolder} className="space-y-2 px-1">
          <input
            ...
            autoFocus
            disabled={isPending}
            className="w-full px-3 py-1.5 bg-slate-50 border border-slate-300 rounded-lg text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:bg-white"
          />
          <button
            type="submit"
            disabled={isPending || !newFolderName.trim()}
            className="w-full py-1.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-medium text-xs rounded-lg transition-colors cursor-pointer"
          >
            {isPending ? "Creating..." : "Save Folder"}
          </button>
          {error && <p className="text-[10px] text-rose-600 px-1">{error}</p>}
        </form>
      )}
      ...
      <nav className="space-y-1">
        ...
        >
          ...
          <span className={`px-2 py-0.5 rounded-full text-[10px] ${!currentFolderId ? "bg-indigo-700 text-white" : "bg-slate-100 text-slate-600"}`}>
            {totalPostsCount}
          </span>
        </Link>
        ...
        >
          ...
          <span className={`px-2 py-0.5 rounded-full text-[10px] ${currentFolderId === "uncategorized" ? "bg-indigo-700 text-white" : "bg-slate-100 text-slate-600"}`}>
            {uncategorizedCount}
          </span>
        </Link>
                ...
                {isDeleting ? (
                  <div className="flex items-center gap-1 px-1">
                    ...
                    <button
                      ...
                      className="px-1.5 py-0.5 text-[9px] bg-slate-200 text-slate-700 rounded"
                    >
                      X
                    </button>
                  </div>
